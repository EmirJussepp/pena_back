<template>
  <div class="container mt-5">
    ...
    <div class="formulario-container">
      <h3 class="form-title">Registrar Tipo de Socio de Peña</h3>

      <form @submit.prevent="crearTipo" class="row g-3" style="margin-top: 1.5rem;">
        <div class="col-md-6">
        ...
        <div class="col-md-6">
          ...
          <input
            id="precio"
            v-model.number="nuevoTipo.precio"
            type="number"
            step="0.01"
            class="form-control"
            required
    ...
    <div class="card shadow-sm p-4 mt-4">
      <h3 class="form-title">Tipos de Socio de Peña</h3>
      ...
      <div class="table-responsive mt-3">
        <table class="professional-table">
          ...
          <tbody>
            <tr v-for="tipo in tipos" :key="tipo.tipoSocioPeñaId">
              <td>{{ tipo.nombre }}</td>
              <td>${{ formatoMoneda(tipo.precio) }}</td>
              <td>
              <button
            ...
            <tr v-if="tipos.length === 0">
              <td colspan="3" class="text-center text-muted py-3">No hay tipos registrados.</td>
            </tr>
          </tbody>
        </table>
      </div>
...
async function obtenerTipos() {
  try {
    const res = await axios.get('http://localhost:8081/sociosPeña')
    tipos.value = res.data
  } catch (error) {
    Swal.fire('Error', 'Error al obtener tipos de socios', 'error')
    console.error(error)
  }
}
...
function iniciarEdicion(tipo) {
  tipoEditandoId.value = tipo.tipoSocioPeñaId
  nuevoTipo.value = { nombre: tipo.nombre, precio: tipo.precio }
}

function cancelarEdicion() {
  tipoEditandoId.value = null
  nuevoTipo.value = { nombre: '', precio: 0 }
}
...
async function crearTipo() {
  ...
  try {
    await axios.post('http://localhost:8081/sociospeña', nuevoTipo.value)
    Swal.fire('Creado', 'Tipo de socio creado exitosamente', 'success')
    cancelarEdicion()
    await obtenerTipos()
  } catch (error) {
...
async function actualizarTipo() {
  try {
    await axios.patch(`http://localhost:8081/sociospeña/precio/${tipoEditandoId.value}`, {
      tipoSocioPeñaId: tipoEditandoId.value,
      nombre: nuevoTipo.value.nombre,
      precio: nuevoTipo.value.precio
    })
    Swal.fire('Actualizado', 'Tipo de socio actualizado correctamente', 'success')
    cancelarEdicion()
    await obtenerTipos()
  } catch (error) {
...
async function eliminarTipo(id) {
  ...
  try {
    await axios.delete(`http://localhost:8081/sociospena/${id}`)
